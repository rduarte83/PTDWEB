<div class="container center-block">
    <h2>Perfil do utilizador</h2>

    <div class="col-md-12 shadow-lg p-3 mb-5 bg-white rounded align-content-center">
        <div class="row">
            <div class="col-md-6">

                <div class="col-md-6 pull-left">
                    <img src="data:image/png;base64," alt="Profile photo.">
                </div>

                <div class="col-md-6 pull-right">
                    <h5>Nome de utilizador</h5>
                    <span>{{'$user_name'}}</span>
                    <br><br>
                    <h5>Email</h5>
                    <span>{{'$email'}}</span>
                    <br><br>
                    <h5>NIF</h5>
                    <span>{{'$n_contribuinte'}}</span>
                    <br><br>
                    <h5>Morada</h5>
                    <span>{{'$morada'}}</span>
                    <br><br>

                    <!-- A função onclick serve apenas para o protótipo. -->
                    <button type="submit" onclick="window.location.href='{{URL("perfil/editar")}}'" class="flex-c-m size1 bg1 bo-rad-20 hov1 s-text1 trans-0-4">Editar perfil</button>
                </div>
            </div>
        </div>
    </div>
</div>
