<div class="container col-md-12 offset-md-2 shadow-lg p-3 mb-5 bg-white rounded">
    <h2>Editar perfil</h2>
    <br>
    
    <div class="modal-body row">
        <div class="col-md">
            <img src="data:image/png;base64," alt="Profile photo.">
            <br>
            <a href="#">Carregar nova imagem</a>
            <br><br>
        </div>
        <div class="col-md">
            <input type="text" placeholder="Nome de utilizador">
            <br><br>
            <input type="text" placeholder="Email">
            <br><br>
            <input type="text" placeholder="NIF">
            <br><br>
            <input type="text" placeholder="Morada">
            <br><br>
            <input type="password" placeholder="Password">
            <br><br>
            <input type="password" placeholder="Confirmar password">
            <br><br><br>
            
            <button type="submit" class="flex-c-m size1 bg1 bo-rad-20 hov1 s-text1 trans-0-4">Guardar alterações</button>
            <br>
            <button type="submit" onclick="window.location.href='/perfil'" class="flex-c-m size1 bg1 bo-rad-20 hov1 s-text1 trans-0-4">Cancelar</button>
        </div>
        <div class="col-md"></div>
        <div class="col-md"></div>
    </div>
</div>